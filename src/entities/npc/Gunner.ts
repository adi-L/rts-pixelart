...
import {
  COLOR_GUNNER, GUNNER_RANGE, GUNNER_FIRE_RATE,
...
export class Gunner extends BaseNPC {
  private state: GunnerState = GunnerState.Hunt;
  ...
  constructor(
    ...
    this.onNightWarning = () => {
      this.state = GunnerState.Returning;
    };

    this.onNightStart = () => {
      this.state = GunnerState.Defend;
      this.sprite.body.setVelocityX(0);
      this.sprite.setPosition(this.towerX, this.sprite.y);
    };

    this.onDayStart = () => {
      this.state = GunnerState.Hunt;
  ...
  private huntBehavior(time: number): void {
    ...
    if (time - this.lastHuntCoinTime >= ARCHER_HUNT_COIN_INTERVAL) {
      this.economy.addCoins(1, 'gunner-hunt');
  ...
  private defendBehavior(time: number): void {
    ...
    for (const child of children) {
      ...
      if (dist <= GUNNER_RANGE && dist < nearestDist) {
        nearestDist = dist;
        nearestZombie = zombie;
      }
    }

    // Fire bullet at nearest zombie if cooldown elapsed
    if (nearestZombie && time - this.lastFireTime >= GUNNER_FIRE_RATE) {
      fireBullet(
        this.bulletPool,
        this.sprite.x,
